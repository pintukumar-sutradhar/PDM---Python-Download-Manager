...
class ProgressBarDelegate(QStyledItemDelegate):
    def paint(self, painter, option, index):
        if index.column() == 3: 
            ...
            opts.rect = option.rect.adjusted(12, 10, -12, -10)
...
class PDMMainWindow(QMainWindow):
    def __init__(self):
        ...
        # 1. Hardware-Accelerated Static Background Layer
    ...
    def _update_background(self, theme_name):
        """Updates the background pixmap. This is fast and prevents flickering."""
        theme_files = {
    ...
    def setup_signals(self):
        ...
        self.engine.download_finished.connect(lambda id, path: self.load_downloads())
        self.engine.status_updated.connect(lambda id, status: self.load_downloads())
        self.engine.metadata_updated.connect(lambda id, size: self.load_downloads())
    ...
    def _add_task(self):
        ...
        if dlg.exec():
            for f in dlg.get_selected_files()['files']:
                self.engine.add_download(f['url'], f['name'], dlg.get_selected_files()['path'], is_audio=f.get('is_audio'), auth=f.get('auth'))
            self.load_downloads()
    ...
    def _delete_selected(self):
        ...
        if QMessageBox.question(self, "Delete", "Move to Trash?" if is_trash else "Delete permanently?") == QMessageBox.StandardButton.Yes:
    ...
    def _clear_list(self):
        if QMessageBox.question(self, "Clear", "Clear history?") == QMessageBox.StandardButton.Yes:
            self.db.clear_all_downloads(); self.load_downloads()
