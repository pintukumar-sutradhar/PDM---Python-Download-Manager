...
class GenericVideoExtractor:
    """PDM's internal module for extracting video links from any generic HTML page."""
    ...
    def extract_videos(url):
        ...
        try:
            ...
            # 2. Search in <a> tags for direct video files
            for a in soup.find_all('a'):
                href = a.get('href')
                if href and any(href.lower().split('?')[0].endswith(ext) for ext in video_exts):
                    videos.append(href)

            # 3. Search in <iframe> tags for embedded players (basic detection)
            for iframe in soup.find_all('iframe'):
                src = iframe.get('src')
                if src and ('.mp4' in src.lower() or 'm3u8' in src.lower()):
                    videos.append(src)

            # Clean and absolute-ify URLs
            results = []
            seen_urls = set()
            for v_url in videos:
